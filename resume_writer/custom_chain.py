...
class CustomSequentialChain:
    ...
    def run(self, verbose: bool = False, include_all_outputs: bool = False, **all_inputs):
        ...
        for i, chain in enumerate(self.chains):
            ...
            output_str = self.__clean_json(output_str)  # TODO: Figure out why is it failing here and clean it
    ...
    def __clean_json(self, output_str: str) -> str:
        cleaned_string = re.sub(r'\s+', ' ', output_str)
        cleaned_string = cleaned_string.replace('"', '\\"')

        # Step 3: Add proper JSON structure
        cleaned_string = cleaned_string.replace('\\"', '"')
        cleaned_string = cleaned_string.replace('",', '",\n')
        cleaned_string = cleaned_string.replace('": ', '":\n')
        cleaned_string = cleaned_string.replace(', "', ',\n"')
